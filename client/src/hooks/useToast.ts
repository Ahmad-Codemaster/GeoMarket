...
function useToast() {
  ...
  React.useEffect(() => {
    const listener: React.Dispatch<Action> = () => setState(reducer(memoryState, { type: 'REMOVE_TOAST' }));

    // Simpler approach: just sync state
    const syncListener = () => setState({ ...memoryState });
    listeners.push(syncListener);
    return () => {
      const index = listeners.indexOf(syncListener);
